...
function retry_extract_schedule() {
...
window.onload = function () {
	document.getElementById("retry-button").onclick = retry_extract_schedule;
	retry_extract_schedule()
};
...
async function update_table() {
	chrome.identity.getAuthToken({
		...
		// 	if (!document.getElementById('authenticate-button')) {
		...
		if (!document.getElementById("export-ics-button")) {
			document.querySelector('#button-div').innerHTML += "<br>" + exportToIcsButtonHTML
		}

		if (document.getElementById('authenticate-button')) {
			document.getElementById('authenticate-button').onclick = authenticate
		}
		...
		if (document.getElementById("export-ics-button")) {
			document.getElementById("export-ics-button").onclick = function () {
...
function importEvents(calId, token, courseEventInfo, semEndDate) {
	...
	for (var i = 0; i < courseEventInfo.length; i++) {
		...
		var params = {
			...
			"description": course.meeting_details,
